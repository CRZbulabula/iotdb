...
/**
 * Computes the RIPEMD-160 hash of an array of bytes. Not instantiable. Refer from "<a
 * href="https://github.com/nayuki/Bitcoin-Cryptography-Library/blob/master/java/io/nayuki/bitcoin/crypto/Ripemd160.java">...</a>"
 */
public final class Ripemd160 {
  ...
  /**
   * Computes and returns a 20-byte (160-bit) hash of the specified binary message. Each call will
   * return a new byte array object instance.
   *
  ...
  public static byte[] getHash(byte[] msg) {
    ...
    for (int i = 0; i < result.length; i++) result[i] = (byte) (state[i / 4] >>> (i % 4 * 8));
    return result;
  }
  ...
  private static void compress(int[] state, byte[] blocks, int len) {
    if (len % BLOCK_LEN != 0) throw new IllegalArgumentException();
    for (int i = 0; i < len; i += BLOCK_LEN) {

      // Message schedule
      int[] schedule = new int[16];
      for (int j = 0; j < BLOCK_LEN; j++) schedule[j / 4] |= (blocks[i + j] & 0xFF) << (j % 4 * 8);
